...
These days I'm mainly focused on [**the last harness**](https://github.com/diegopetrucci/the-last-harness) — an opinionated [pi](https://github.com/earendil-works/pi-mono)-based agent harness with, at its core, an orchestrator > subagents workflow, adversarial reviews from different providers, self-improvement with a native memory system, and flawless context-management. Skate where the puck is going, not where it used to be.
